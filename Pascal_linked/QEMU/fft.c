...
ifft( complex *v, int n, complex *tmp )
{
  if(n>1) {			/* otherwise, do nothing and return */
    ...
    for(m=0; m<n/2; m++) {
      w.Re = cos(2*2*PI*m/(double)n);
      w.Im = sin(2*2*PI*m/(double)n);
      z.Re = w.Re*vo[m].Re - w.Im*vo[m].Im;	/* Re(w*vo[m]) */
      z.Im = w.Re*vo[m].Im + w.Im*vo[m].Re;	/* Im(w*vo[m]) */
      v[  m  ].Re = ve[m].Re + z.Re;
      v[  m  ].Im = ve[m].Im + z.Im;
...
fft_pas(void)
{
  ...
  for(k=0; k<N; k++) {
    v[k].Re = 0.75*cos(80*2*PI*k/(double)N)+0.25*cos(40*2*PI*k/(double)N);
    v[k].Im = 0.75*sin(80*2*PI*k/(double)N)+0.25*sin(40*2*PI*k/(double)N);
